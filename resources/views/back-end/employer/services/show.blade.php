            ...
            <div class="wt-dashboardbox">
                ...
                <div class="wt-dashboardboxcontent wt-jobdetailsholder">
                    ...
                    <div class="wt-service-tabel wt-jobservice-details">
                        @if (!empty($freelancer))
                        @if (!empty($attachment))
                        <figure class="service-feature-image"><img
                                src="{{{asset('/uploads/services/'.$freelancer->id.'/'.$attachment[0])}}}"
                        ...
                        <div class="wt-freelancers-content">
                            <div class="dc-title">
                                ...
                                @endif
                                <h3>{{{$service['title']}}}</h3>
                                <span><strong>{{ !empty($symbol) ? $symbol['symbol'] : '£' }}{{{$service['price']}}}</strong>
                                    {{ trans('lang.starting_from') }}</span>
                            </div>
                        </div>
                    </div>
                    @else
                                ...
                                <div class="wt-userlistingcontent">
                                    <div class="wt-contenthead">
                                        ...
                                        <ul class="wt-userlisting-breadcrumb">
                                            ...
                                            <li><span><i class="far fa-money-bill-alt"></i>
                                                    {{ !empty($symbol) ? $symbol['symbol'] : '£' }}{{{ $service->price }}}</span>
                                            </li>
                                            @endif
                                    ...
                                    <div class="wt-rightarea">
                                        <div class="wt-hireduserstatus">
                                            <figure><img
                                                    src="{{{ asset(Helper::getImage('uploads/users/'.$purchaser->id, $purchaser->profile->avater, 'medium-small-', 'user.jpg')) }}}"
                                                    alt="{{ trans('lang.profie_img') }}"></figure>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="wt-contenthead">
                                        <div class="wt-title">
                                            <a href="{{{url('profile/'.$freelancer->slug)}}}">
                                                {{{Helper::getUserName($freelancer->id)}}}</a>
                                        </div>
                                    </div>
                                        ...
                                        <fieldset>
                                            <div class="form-group">
                                                <span class="wt-select">
                                                    {!! Form::select('status', $service_status, 'hired', array('id'
                                                    =>'employer_service_status', 'data-placeholder' =>
                                                    trans('lang.select_status'), '@change' =>
                                                    'serviceStatus('.$service->id.', '.$pivot_id.',
                                                    '.Auth::user()->id.', "'.$cancel_proposal_text.'",
                                                    "'.$cancel_proposal_button.'", "'.$validation_error_text.'",
                                                    "'.$cancel_popup_title.'")')) !!}
                                                </span>
                                                <a href="javascrip:void(0);" class="wt-searchgbtn job_status_popup"
                                                    @click.prevent='serviceStatus({{$service->id}}, {{$pivot_id}}, {{Auth::user()->id}}, "{{$cancel_proposal_text}}", "{{$cancel_proposal_button}}", "{{$validation_error_text}}", "{{$cancel_popup_title}}")'><i
                    ...
                    <div class="wt-projecthistory">
                        ...
                        <div class="wt-historycontent la-jobdetails-holder">
                            <private-message :placeholder="'{{ trans('lang.ph_job_dtl') }}'"
                                :upload_tmp_url="'{{url('service/upload-temp-message_attachments')}}'"
                                :id="'{{$pivot_id}}'" :recipent_id="'{{$freelancer->id}}'" :project_type="'service'">
                            </private-message>
                        </div>
                    </div>
                    @endif
                ...
                <fieldset>
                    ...
                    <div class="form-group wt-btnarea">
                        <a class="wt-btn" href="javascript:void(0);"
                            v-on:click='submitFeedback({{$freelancer->id}}, {{$pivot_id}})'>{{ trans('lang.btn_send_feedback') }}</a>
                    </div>
                </fieldset>
            </form>
        </div>
