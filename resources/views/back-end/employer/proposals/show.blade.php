...
<section class="wt-haslayout wt-dbsectionspace" id="jobs">
    ...
    <div class="row">
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
            ...
            <div class="flash_msg">
                <flash_messages :message_class="'success'" :time='5' :message="'{{{ Session::get('success') }}}'"
                ...
                <div class="wt-dashboardboxcontent wt-jobdetailsholder">
                                    ...
                                    <div class="wt-contenthead">
                                        ...
                                        <div class="wt-title">
                                            ...
                                            <a href="{{{ url('profile/'.$job->employer->slug) }}}">@if ($verified_user
                                                === 1)<i
                                                    class="fa fa-check-circle"></i>@endif&nbsp;{{{ $employer_name }}}</a>
                                            @endif
                                            @if (!empty($job->title))
                                            <h2>{{{ $job->title }}}</h2>
                                            @endif
                        ...
                        <div class="wt-jobdetailscontent">
                            ...
                            <div class="wt-userlistinghold wt-featured wt-proposalitem">
                                <figure class="wt-userlistingimg">
                                    <img src="{{{ asset($profile_image) }}}" alt="{{ trans('lang.is_featured') }}"
                                ...
                                <div class="wt-rightarea wt-titlewithsearch">
                                    @if ($job->status === 'hired' && Auth::user()->getRoleNames()->first() ==
                                    'employer')
                                    <form class="wt-formtheme wt-formsearch" id="change_job_status">
                                        <fieldset>
                                            <div class="form-group">
                                                <span class="wt-select">
                                                    {!! Form::select('status', $project_status, $job->status, array('id'
                                                    =>'job_status', 'data-placeholder' => trans('lang.select_status'),
                                                    '@change' => 'jobStatus('.$job->id.', '.$accepted_proposal->id.',
                                    ...
                                    <div class="wt-hireduserstatus">
                                        ...
                                        {!! form::close(); !!}
                                        <a href="javascript:void(0);"
                                            v-on:click.prevent="downloadAttachments('{{'download-attachments-form-'.$accepted_proposal->freelancer_id}}')"><span>{{{ $count }}}
                                                {{trans('lang.file_attached')}}</span></a>
                                    </div>
                                    @endif
                                </div>
                            </div>
                    ...
                    <div class="wt-projecthistory">
                        ...
                        <div class="wt-historycontent la-jobdetails-holder">
                            <private-message :placeholder="'{{ trans('lang.ph_job_dtl') }}'"
                                :upload_tmp_url="'{{url('proposal/upload-temp-image')}}'"
                                :id="'{{$accepted_proposal->id}}'"
                                :recipent_id="'{{$accepted_proposal->freelancer_id}}'"></private-message>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <b-modal ref="myModalRef" hide-footer title="Project Status">
        <div class="d-block text-center">
            <form class="wt-formtheme wt-formfeedback" id="submit-review-form">
                <fieldset>
                    <div class="form-group">
                        <textarea class="form-control" placeholder="{{ trans('lang.add_your_feedback') }}"
                    ...
                    <div class="form-group wt-ratingholder">
                        <div class="wt-ratepoints">
                            <vue-stars :name="'rating[{{$key}}][rate]'" :active-color="'#fecb02'"
                                :inactive-color="'#999999'" :shadow-color="'#ffff00'" :hover-color="'#dddd00'" :max="5"
                                :value="0" :readonly="false" :char="'★'" id="rating-{{$key}}" />
                    ...
                    <div class="form-group wt-btnarea">
                        <a class="wt-btn" href="javascript:void(0);"
                            v-on:click='submitFeedback({{$accepted_proposal->freelancer_id}}, {{$job->id}})'>{{ trans('lang.btn_send_feedback') }}</a>
                    </div>
                </fieldset>
            </form>
        </div>
