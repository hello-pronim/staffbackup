        ...
        <div class="row">
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" id="jobs">
                ...
                <div class="wt-dashboardbox">
                    ...
                    $job->employer->id)->pluck('user_verified')->first();
                    $count = 0;
                    ...
                    <div class="wt-dashboardboxcontent wt-rcvproposala">
                        <div class="wt-userlistinghold wt-userlistingvtwo {{ $feature_class }}">
                            ...
                            <div class="wt-userlistingcontent">
                                <div class="wt-contenthead">
                                    ...
                                    <div class="wt-title">
                                        @if (!empty($user_name))
                                        <a href="{{{ url('profile/'.$job->employer->slug) }}}">@if($verified_user ===
                                            1)<i class="fa fa-check-circle"></i>@endif&nbsp;{{{ $user_name }}}</a>
                                        @endif
                                        @if (!empty($job->title))
                                        <h2>{{{ $job->title }}}</h2>
                                        @endif
                                    ...
                                    <ul class="wt-saveitem-breadcrumb wt-userlisting-breadcrumb">
                                        ...
                                        <li><span><img
                                                    src="{{{asset(App\Helper::getLocationFlag($job->location->flag))}}}"
                                                    alt="{{ trans('lang.img') }}"> {{{ $job->location->title }}}</span>
                                        </li>
                                        @endif
                                        @if (!empty($job->project_type))
                                        <li><a href="javascript:void(0);" class="wt-clicksavefolder"><i
                                                    class="far fa-folder"></i> {{ trans('lang.type') }}
                                                {{{ $job->project_type }}}</a></li>
                                        @endif
                                        @if (!empty($job->duration))
                                        <li><span class="wt-dashboradclock"><i class="far fa-clock"></i>
                                                {{ trans('lang.duration') }} {{{ $duration }}}</span></li>
                                        @endif
                                    </ul>
                                    @endif
                                </div>
                                <div class="wt-rightarea">
                                    <div class="wt-hireduserstatus">
                                        ...
                                        <ul class="wt-hireduserimgs">
                                            ...
                                            $profile_image = !empty($user_image) ?
                                            '/uploads/users/'.$proposal->freelancer_id.'/'.$user_image :
                                            'images/user-login.png';
                                            @endphp
                                            <li>
                                                <figure><img src="{{{ asset($profile_image) }}}"
                                                        alt="{{ trans('lang.img') }}" class="mCS_img_loaded"></figure>
                                            </li>
                                            @endforeach
                                        </ul>
                                        @endif
                        ...
                        <div class="wt-freelancerholder wt-rcvproposalholder la-free-proposal">
                            ...
                            <div class="wt-managejobcontent">
                                ...
                                $user_image = !empty($profile) ? $profile->avater : '';
                                $profile_image = !empty($user_image) ?
                                '/uploads/users/'.$accepted_proposal->freelancer_id.'/'.$user_image :
                                'images/user-login.png';
                                $user_name = Helper::getUserName($user->id);
                                $avg_rating = \App\Review::where('receiver_id',
                                $proposal->freelancer_id)->sum('avg_rating');
                                ...
                                $proposal->freelancer_id)->count();
                                $completion_time = !empty($accepted_proposal->completion_time) ?
                                \App\Helper::getJobDurationList($accepted_proposal->completion_time) : '';
                                $p_attachments = !empty($accepted_proposal->attachments) ?
                                unserialize($accepted_proposal->attachments) : '';
                                ...
                                <div class="wt-userlistinghold wt-proposalitem {{ $feature_class }}">
                                    ...
                                    <span class="wt-featuredtag" style="border-top: 40px solid {{ $badge_color }};">
                                        <img src="{{{ asset(Helper::getBadgeImage($badge_img)) }}}"
                                            alt="{{ trans('lang.hired_freelancers') }}" data-tipso="Plus Member"
                                    ...
                                    <div class="wt-rightarea">
                                        <div class="wt-btnarea">
                                            <a href="javascript:void(0);" class="wt-btn"
                                                style="pointer-events:none;">{{ trans('lang.hired') }}</a>
                                            <a href="{{{ url('proposal/'.$job->slug.'/'.$job->status) }}}"
                                                class="wt-btn">{{ trans('lang.view_detail') }}</a>
                                        </div>
                                        <div class="wt-hireduserstatus">
                                            <h5>{{ !empty($symbol) ? $symbol['symbol'] : '£' }}{{{ $accepted_proposal->amount }}}
                                            </h5>
                                        ...
                                        <div class="wt-hireduserstatus">
                                            ...
                                            <a href="javascript:void(0);"
                                                v-on:click.prevent="showCoverLetter('{{ $accepted_proposal->id }}')"><span>{{ trans('lang.cover_letter') }}</span></a>
                                        </div>
                                        <div class="wt-hireduserstatus">
                                            <i class="fa fa-paperclip"></i>
                                            @if (!empty($p_attachments))
                                            {!! Form::open(['url' => url('proposal/download-attachments'), 'class'
                                            ...
                                            {!! form::close(); !!}
                                            <a href="javascript:void(0);"
                                                v-on:click.prevent="downloadAttachments('{{'accepted-download-attachments-form-'.$accepted_proposal->id}}')"><span>{{{ $count }}}
                                                    {{ trans('lang.files_attached') }}</span></a>
                                            @else
                        ...
                        <div class="wt-freelancerholder wt-rcvproposalholder">
                            ...
                            <div class="wt-managejobcontent">
                                ...
                                $user_image = !empty($profile) ? $profile->avater : '';
                                $profile_image = !empty($user_image) ?
                                '/uploads/users/'.$proposal->freelancer_id.'/'.$user_image : 'images/user-login.png';
                                $user_name = $user->first_name.' '.$user->last_name;
                                $avg_rating = \App\Review::where('receiver_id',
                                $proposal->freelancer_id)->sum('avg_rating');
                                $rating = $avg_rating != 0 ? round($avg_rating/\App\Review::count()) : 0;
                                ...
                                $feedbacks = \App\Review::select('feedback')->where('receiver_id',
                                $proposal->freelancer_id)->count();
                                $completion_time = !empty($proposal->completion_time) ?
                                \App\Helper::getJobDurationList($proposal->completion_time) : '';
                                ...
                                <div class="wt-userlistinghold wt-proposalitem {{ $feature_class }}">
                                    ...
                                    <div class="wt-rightarea">
                                        ...
                                        <div class="wt-hireduserstatus">
                                            ...
                                            {!! form::close(); !!}
                                            <a href="javascript:void(0);"
                                                v-on:click.prevent="downloadAttachments('{{'download-attachments-form-'.$proposal->id}}')"><span>{{{ $received_proposal_count }}}
                                                    {{ trans('lang.files_attached') }}</span></a>
                                            @else
                                        ...
                                        <div class="wt-hireduserstatus">
                                            <i class="fa fa-paperclip"></i>
                                            <a target="_blank"
                                                href="<?= url('uploads/cvs/'.$proposal->freelancer->profile->cvFile) ;?>">Download
                                                CV</a>
