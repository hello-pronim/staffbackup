...
<div class="wt-location wt-tabsinfo">
    ...
    <div class="wt-settingscontent">
        @if (!empty($cv))
            @php $image = '/uploads/cvs/'.$cv; @endphp
            <div class="wt-formtheme wt-userform">
                <div v-if="this.uploaded_image">
                    <upload-image
                            :id="'cv'"
                ...
                <div class="wt-uploadingbox" v-else>
                    <figure><img src="{{{asset($image)}}}" alt="{{{ trans('lang.profile_cv') }}}"></figure>
                    <div class="wt-uploadingbar">
                        <div class="dz-filename">{{{$cv}}}</div>
                        <em>{{{ trans('lang.file_size') }}}<a href="javascript:void(0);" class="lnr lnr-cross" v-on:click.prevent="removeImage('hidden_avater')"></a></em>
                    </div>
                </div>
                <input type="hidden" name="hidden_cv_image" id="hidden_cv" value="{{{$cv}}}">
            </div>
        @else
            <div class="wt-formtheme wt-userform">
                <upload-image
