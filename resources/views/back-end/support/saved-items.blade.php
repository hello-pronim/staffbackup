                ...
                <div class="wt-dashboardtabs">
                    <ul class="wt-tabstitle nav navbar-nav">
                        ...
                        <li class="nav-item"><a data-toggle="tab"
                                href="#wt-education">{{ trans('lang.followed_companies') }}</a></li>
                        <li class="nav-item"><a data-toggle="tab"
                                href="#wt-awards">{{ trans('lang.liked_freelancers') }}</a></li>
                    </ul>
                </div>
                            ...
                            <div class="wt-dashboradsaveitem">
                                ...
                                <div class="wt-userlistinghold wt-featured wt-dashboradsaveditems">
                                    ...
                                    <span class="wt-featuredtag"><img src="{{{ asset('images/featured.png') }}}"
                                            alt="{{ trans('lang.img') }}" data-tipso="Plus Member"
                                            ...
                                            <div class="wt-contenthead wt-followcomhead">
                                                <div class="wt-title">
                                                    <a href="{{{ url('profile/'.$emp->slug) }}}">
                                                        ...
                                                        {{ trans('lang.verified_company') }}</a>
                                                    @endif
                                                    <h3><a
                                                            href="{{ url('profile/'.$emp->slug) }}">{{{ $user_name }}}</a>
                                                    </h3>
                                                </div>
                                                <ul class="wt-followcompomy-breadcrumb wt-userlisting-breadcrumb">
                                                    <li><a href="{{{ url('profile/'.$emp->slug) }}}">
                                                            {{ trans('lang.open_jobs') }} </a></li>
                                                    <li><a href="{{{ url('profile/'.$emp->slug) }}}">
                                                            {{ trans('lang.full_profile') }}</a></li>
                                                    <li><a href="javascript:void(0);" class="wt-savefollow">
                                                            {{ trans('lang.following') }}</a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="wt-awardsholder tab-pane fade" id="wt-awards">
                        <div class="wt-addprojectsholder wt-likefreelan la-likefreelanfav">
                            ...
                            <div class="wt-likedfreelancers wt-haslayout">
                                ...
                                $user_image = !empty($profile) ? $profile->avater : '';
                                $profile_image = !empty($user_image) ? '/uploads/users/'.$freelancer.'/'.$user_image :
                                'images/user.jpg';
                                $user_name = Helper::getUserName($freelancer);
                                $reviews = \App\Review::where('receiver_id', $freelancer)->count();
                                $proposal = \App\Proposal::select('job_id')->where('freelancer_id',
                                $user->id)->pluck('job_id')->first();
                                $employer_id = !empty($proposal) ? \App\Job::select('user_id')->where('id',
                                $proposal)->pluck('user_id')->first() : '';
                                $job = !empty($employer_id) ? \App\Job::where('user_id', $employer_id)->first() : '';
                                $featured_class = (!empty($job) && $job->is_featured == 'true') ? 'wt-featured' : '';
                                $verified_user = \App\User::select('user_verified')->where('id',
                                $freelancer)->pluck('user_verified')->first();
                                ...
                                $stars = $user_reviews->sum('avg_rating') != 0 ? $user_reviews->sum('avg_rating')/20*100
                                : 0;
                                @endphp
                                        ...
                                        <div class="wt-contenthead">
                                            <div class="wt-title">
                                                <a href="{{{ url('profile/'.$user->slug) }}}">
                                                    ...
                                                    {{{ $user_name }}}</a>
                                                <h2><a
                                                        href="{{ url('profile/'.$user->slug) }}">{{{ html_entity_decode($profile->tagline, ENT_QUTOES) }}}</a>
                                                </h2>
                                            </div>
                                            <ul class="wt-userlisting-breadcrumb">
                                                @if (!empty($profile->hourly_rate))
                                                <li><span><i class="far fa-money-bill-alt"></i>
                                                        {{ !empty($symbol) ? $symbol['symbol'] : '£' }}{{{ $profile->hourly_rate }}}
                                                        {{ trans('lang.per_hour') }}</span>
                                                </li>
                                                @endif
                                                <li><span><img
                                                            src="{{{asset(Helper::getLocationFlag($user->location->flag))}}}"
                                                            alt="{{{ trans('lang.locations') }}}">
                                                        {{{ $user->location->title }}}</span></li>
                                                <li class="wt-btndisbaled"><a href="javascript:void(0);"
                                                        class="wt-clicksave"><i class="fa fa-heart"></i> Saved</a></li>
                                            </ul>
                                        </div>
