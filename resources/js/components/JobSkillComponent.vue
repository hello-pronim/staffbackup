  ...
  <div>
    <div class="wt-formtheme wt-skillsform">
      <transition name="fade">
        <div v-if="isShow" class="sj-jump-messeges">
          {{ trans("lang.no_record") }}
        </div>
      </transition>
      <fieldset>
        ...
        <div class="form-group wt-btnarea">
          <a href="javascript:void(0);" class="wt-btn" @click="addSkill">{{
            trans("lang.add_skills")
          }}</a>
        </div>
      </fieldset>
    </div>
    <div class="wt-myskills">
      <ul id="skill_list" class="sortable list" v-if="job_skills">
        <li
          v-for="(job_skill, index) in job_skills"
          :key="index"
          class="skill-element"
        >
          ...
          <span class="skill-dynamic-html"> {{ job_skill.title }}</span>
          <span class="skill-dynamic-field sss">
            <input
              type="hidden"
              v-bind:name="'skills[' + index + '][id]'"
              :value="job_skill.id"
            />
          </span>
...
export default {
  ...
  methods: {
    ...
    //         self.professions = response.data.skills;
    //         if(self.professions.length == 0) {
    ...
    addSkill: function() {
      if (this.is_empty == false) {
        ...
        if (skillsSelect.options[skillsSelect.selectedIndex]) {
          this.selected_skill_text =
            skillsSelect.options[skillsSelect.selectedIndex].text;
          this.selected_skill = document.getElementById(
            "freelancer_skill"
          ).value;
          this.skills.push(
            Vue.util.extend(
              ...
              this.skill,
              this.skill.count++,
              (this.skill.title = this.selected_skill_text),
              (this.skill.id = this.selected_skill),
              (this.skill.rating = this.selected_rating)
            )
          );
    ...
    getJobSkills() {
      ...
      if (edit_url == "edit-job") {
        var slug = segment_array[segment_array.length - 1];
        axios
          .post(APP_URL + "/job/get-stored-job-skills", {
            slug: slug,
          })
          .then(function(response) {
  ...
  mounted: function() {
    jQuery(document).on("click", ".wt-skillsactive", function(e) {
      ...
      var edit_skill_value = _this
        .parents("li")
        .find(".skill-dynamic-field input:text")
        .val();
      _this
        .parents("li")
        .find(".skill-dynamic-html em")
        .html(edit_skill_value);
    });
    this.getProfessions();
    //this.getSkills();
    this.getJobSkills();
