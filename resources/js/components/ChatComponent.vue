    ...
    <div class="wt-dashboardboxtitle wt-titlemessages" v-if="user">
      ...
      <div class="wt-userlogedin">
        ...
        <div class="wt-username">
          <h3>
            <i class="fa fa-check-circle"></i> {{ user.selected_user_name }}
          </h3>
          <span>{{ user.selected_user_tagline }}</span>
        </div>
      </div>
      <a
        :href="url + '/profile/' + user.selected_user_slug"
        class="wt-viewprofile"
        >{{ trans("lang.view_profile") }}</a
      >
    </div>
    <div class="wt-dashboardboxcontent wt-dashboardholder wt-offersmessages">
      ...
      <div class="wt-chatarea wt-chatarea-empty" v-else>
        <figure class="wt-chatemptyimg">
          ...
          <figcaption>
            <h3>{{ trans("lang.no_chat_message") }}</h3>
          </figcaption>
        </figure>
      </div>
    </div>
...
export default {
  ...
  methods: {
    getUsers() {
      let self = this;
      axios.get(APP_URL + "/message-center/get-users").then(function(response) {
